...
from data_utils import NN_DataHelper, postprocess, preprocess, train_info_args
from deep_training.data_helper import (DataArguments, ModelArguments,
                                       TrainingArguments)
from deep_training.nlp.models.chatglm import (ChatGLMConfig,
                                              TransformerChatGlmLMHeadModel,
                                              setup_model_profile)
...
class MySimpleModelCheckpoint(SimpleModelCheckpoint):
    ...
    @staticmethod
    def generate_text(
            pl_module: MyTransformer, prompt_text, tokenizer: ChatGLMTokenizer,
            max_target_length, device=0):
    ...
    def on_save_model(
        ...
        for text in prefixs:
            ...
            output = MySimpleModelCheckpoint.generate_text(
                pl_module, input_text, tokenizer, data_args.max_target_length, device=device)
...
if __name__ == '__main__':
    ...
    if data_args.do_train:
        dataHelper.make_dataset_with_args(
            data_args.train_file, mixed_data=False, shuffle=True, mode='train')
    ...
    if not data_args.convert_onnx:
        ...
        train_datasets = dataHelper.load_random_sampler(
            dataHelper.train_files, with_load_memory=True,
            collate_fn=dataHelper.collate_fn,
            batch_size=training_args.train_batch_size, shuffle=True,
            infinite=True, num_processes=trainer.world_size,
